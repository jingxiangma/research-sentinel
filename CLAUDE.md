...
The scheduled task executes `run_pipeline.bat`, which runs the pipeline scripts in order and commits/pushes the results. Logs are written to `logs/pipeline.log`.

To create or refresh the scheduled task on a machine:
- **Register:** `powershell -ExecutionPolicy Bypass -File .\setup_automation.ps1`
