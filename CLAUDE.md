...
The pipeline runs automatically every Monday at 9:00am via **Windows Task Scheduler** (task name: `ResearchSentinel_WeeklyUpdate`). If the computer is off at that time, it runs on next boot (`StartWhenAvailable`).
